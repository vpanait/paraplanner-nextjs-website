export const ROUTE = {
    HOME: '/',
    SECURITY: '/security',
    COMPANY: '/company',
    TEAM: '/team',
    ABOUT: '/about',
    FAQ: '/faq',
    TERMS: '/terms-of-service',
    PRIVACY: '/privacy-policy',
    SUPPORT: '/support',
    DOCUMENTATION: '/documentation',
}

export const APP_SIGN_IN_URL = "https://app.paraplanner.ai/";
export const KITCES_URL = "https://www.kitces.com/fintechmap/";

export const CONTENT_WRAPPER_PADDING_Y = 120;

export const CONTENT_WRAPPER_VISIBLE_ANIMATION = {
    y: 0,
    opacity: 1,
};

export const CONTENT_WRAPPER_HIDDEN_ANIMATION = {
    y: CONTENT_WRAPPER_PADDING_Y * 0.8,
    opacity: 0,
};

// TODO: justin links
export const CALENDARLY: {
    [key: string]: {
        GET_STARTED: string;
        BOOK_INTRO_CALL: string;
        BOOK_A_CALL: string;
    };
} = {
    DEFAULT: {
        GET_STARTED: "https://calendly.com/paraplanner-ai-seth-lead-paraplanner/paraplanner-ai-meeting",
        BOOK_INTRO_CALL: "https://calendly.com/paraplanner-ai-seth-lead-paraplanner/paraplanner-ai-meeting",
        BOOK_A_CALL: "https://calendly.com/paraplanner-ai-seth-lead-paraplanner/paraplanner-ai-meeting",
    },
    JUSTIN: {
        GET_STARTED: "https://calendly.com/paraplanner-ai-seth-lead-paraplanner/paraplanner-ai-meeting",
        BOOK_INTRO_CALL: "https://calendly.com/paraplanner-ai-seth-lead-paraplanner/paraplanner-ai-meeting",
        BOOK_A_CALL: "https://calendly.com/paraplanner-ai-seth-lead-paraplanner/paraplanner-ai-meeting",
    }
}
export const CONTACT = {
    EMAIL: "hello@paraplanner.ai",
    PHONE: "+17373733370",
    PHONE_DISPLAY: "(737) 373-3370",
    LINKEDIN: 'https://linkedin.com/company/paraplannerai',
    TWITTER: 'https://twitter.com/paraplannerai',
    TERMS: 'https://docs.google.com/document/d/1ux_Fa1C688s_Q8knfiDp9lQuTS0-tQyP/edit?usp=sharing&ouid=110062069340887754675&rtpof=true&sd=true',
    PRIVACY: 'https://docs.google.com/document/d/1XzDV4J_Vl85oh5fMuuhMPARfnGgEV4u8/edit?usp=sharing&ouid=110062069340887754675&rtpof=true&sd=true'
}

export const EVENT = {
    HOME: {
        HERO: {
            GET_STARTED: 'home_hero_get-started',
            BOOK_CALL: 'home_hero_book-intro-call',
            TECH_MAP: 'home_hero_tech-map',
        },

        SOLUTION: {
            SLIDER_SWIPE: 'home_solution_slider-swipe',
            TAB_CLICK: 'home_solution_tab-click',
        },
        PLAN: {
            GET_STARTED: 'home_plan_get-started',
            BOOK_INTRO_CALL: 'home_plan_book-intro-call',
        },
        CONTACT: {
            CALL_NOW: 'home_contact_call-us-now',
            BOOK_CALL: 'home_contact_book-call',
            VIA_EMAIL: 'home_contact_contact-via-email',
        }
    },
    FOOTER: {
        CONTACT_EMAIL: 'footer_contact_via-email',
        CONTACT_PHONE: 'footer_contact_via-phone',
        CONTACT_SUPPORT: 'footer_contact_support',
        FOLLOW_LINKEDIN: 'footer_follow_linkedin',
        FOLLOW_X: 'footer_follow_x',
        COMPANY_TEAM: 'footer_company_team',
        COMPANY_ABOUT: 'footer_company_about',
        COMPANY_FAQ: 'footer_company_faq',
        COMPANY_DOCUMENTATION: 'footer_company_documentation',
        LEGAL_TERMS: 'footer_legal_terms',
        LEGAL_PRIVACY: 'footer_legal_privacy',
    }
}
